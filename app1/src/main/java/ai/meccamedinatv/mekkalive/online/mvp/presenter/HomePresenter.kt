...
import com.walhalla.ytlib.R as coreR
...
class HomePresenter : MvpPresenter<HomeView>(),
    Repository.Callback<GenericJson> {
    ...
    init {
        ...
        context = DexApplication.instance
...
//                .build();
    ...
    override fun attachView(view: HomeView?) {
        ...
        getResultsFromApi(DexApplication.instance)
    }
...
//AlQuranHD        }https://www.youtube.com/channel/UCraPI8sg-eiNzUrurxhKeEQ
    ...
    fun checkAndSendRequest(context: Context) {
    ...
    private fun loadFromFirebase(tryOtherRepo: Boolean) {
        firebaseRepository.loadPlaylist(object : Repository.Callback<List<ListEntryUI>> {
            override fun onMessageRetrieved(listEntries: List<ListEntryUI>) {
                if (mIsAppFirstLaunched) {
                    DLog.d("@@@@" + listEntries.size + "@@" + Thread.currentThread())
                    this@HomePresenter.uiList = LinkedList(listEntries)
...
//                }
            ...
            //Save
            if (mIsAppFirstLaunched && this@HomePresenter.uiList != null && noneNullState()) {
    ...
    private fun loadFromYouTube() {
        mExecutorService.submit {
            val CHANNEL_ID =
                context.resources.getStringArray(R.array.channel_ids)
    ...
    fun fragmentConnected() {
        if (uiList != null) {
            ...
            //channelVideosPresenter.getVideoData(data);
        }
    }
     ...
     * @param entry
     */
    fun getVideoData(entry: List<ListEntryUI>, position: Int) {
...
//        };
        ...
        when (type) {
            ...
            Const.TYPE_PLAY_SEARCH -> {
                ...
                //DLog.d(("@@@" + R.integer._PLAY_LIST + " " + R.integer._SEARCH + " @ False video type: " + entry[position].toString()))
            }
        }
        DLog.d("@@@-$type")
    }
