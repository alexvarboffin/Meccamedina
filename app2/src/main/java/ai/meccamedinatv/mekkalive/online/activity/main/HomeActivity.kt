package ai.meccamedinatv.mekkalive.online.activity.main

import ai.meccamedinatv.mekkalive.online.BuildConfig
import ai.meccamedinatv.mekkalive.online.R
import ai.meccamedinatv.mekkalive.online.activity.about.ActivityAbout
import ai.meccamedinatv.mekkalive.online.config.Const
import ai.meccamedinatv.mekkalive.online.databinding.ActivityHomeBinding
import ai.meccamedinatv.mekkalive.online.fragment.Tasbeeh
...
class HomeActivity : MvpAppCompatActivity(), HomeView, BaseFragment.Callback,
    ChannelVideosCallback,
    PermissionCallbacks, VideoFragment.VideoFragmentCallback {
            ...
            drawer = DrawerBuilder(this@HomeActivity)
                ...
                .addStickyDrawerItems(
                    ...
                    SecondaryDrawerItem()
                        ...
                        .withSelectable(false),  //                            new SecondaryDrawerItem()
...
//        mFragment.setArguments(bundle);
    ...
    private fun replaceFragment0(fragment: Fragment) {
        supportFragmentManager
            .beginTransaction()
...
//        ft.commit();
    ...
    private fun loadBannerAd() {
        ...
        //Toast.makeText(this, "@@@@" + isFullscreen, Toast.LENGTH_LONG).show();
        binding!!.adView.visibility = View.VISIBLE
        if (BuildConfig.DEBUG) {
            Toast.makeText(this, "binding.adView load", Toast.LENGTH_SHORT).show()
        }
    }

    private fun cancelBannerLoading() {
        binding!!.adView.visibility = View.GONE
        binding!!.adView.pause() // Остановка загрузки, чтобы не показывать новые рекламные объявления
        DLog.d("{binding.adView canceled}")
        if (BuildConfig.DEBUG) {
    ...
    private fun resumeBannerLoading() {
        binding!!.adView.resume() // Возобновление загрузки, чтобы показывать новые рекламные объявления
        binding!!.adView.visibility = View.VISIBLE
        if (BuildConfig.DEBUG) {
            Toast.makeText(this, "resumeBanner", Toast.LENGTH_SHORT).show()
        }
    }
...
//        }
    ...
    override fun chooseAccount(credential: GoogleAccountCredential?) {
